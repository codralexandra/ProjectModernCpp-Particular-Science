...
template<typename T>
void http::Routing<T>::LobbyWebSocketHandler(crow::websocket::connection& con)
{
	if (m_game.GetLobbyState() == LobbyState::Starting)
	{
		con.send_text("Game is starting");
	}

	if (m_game.GetLobbyState() == LobbyState::Waiting)
	{
		con.send_text("Game is waiting");
	}
}
...
void http::Routing<int>::Run(Storage& storage)
{
	...
	CROW_ROUTE(m_app, "/lobby/waiting/socket").websocket().onopen([&](crow::websocket::connection& con) {
		LobbyWebSocketHandler(con);
		});
